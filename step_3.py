...
def step_3(ai_model, agent_file_path, season_file_path):
    ...
    # 2.2: load the season yaml file
    season_data = None    
    with open(season_file_path, "r", encoding="utf-8") as file:
        season_data = yaml.safe_load(file)

    # step 3.3: Generate a new season name, topic, and communication style with the prompt_2 template
    season_template = manager.create_new_template_yaml(TemplateType.SEASON)

    # we only want to send the details about the season and not all the episodes
    ...
    # 2. Access the 'episodes' key under 'season'
    episodes = season_data["season"]["episodes"]

    # 3. Loop through each episode in the list
    for episode in episodes:
        # 3.1: create a new episode template
        episode_template = manager.create_new_template_yaml(TemplateType.EPISODE)
    
        # step 3.2: file in template
        ...
        episode_template["episode"]["episode_overview"] = episode["episode_overview"]
        ...
        episode_file_path = manager.create_filepath(
            agent_name=agent_yaml["name"],
            season_number=season_data["season"]["season_number"],
